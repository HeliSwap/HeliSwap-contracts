...
task("approveHTS", "Approves an HTS token for spending by an account")
  ...
  .addParam(
    "spenderaccountid",
    "The account that will be permitted to spend tokens"
  )
...
task("deployWrapper", "Deploys the Router Wrappper")
  .addParam("whbar", "The WHBAR address to use")
  .addParam("router", "The current router address to use")
  .setAction(async (taskArgs) => {
    const deployment = require("./scripts/deployWrapper");
